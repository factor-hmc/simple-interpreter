...
let rec eval = (stack: stack, word: word): result(string, stack) =>
  ...
  | (If, [Quotation(when_false), Quotation(when_true), b, ...st]) =>
    to_bool(b) ? eval_words(st, when_true) : eval_words(st, when_false)
  | (Dup, [x, ...st]) => Ok([x, x, ...st])
...
and eval_result =
    (stack: result(string, stack), word: word): result(string, stack) =>
  switch (stack) {
  | Error(e) => Error(e)
  | Ok(st) => eval(st, word)
  }
and eval_words = (stack: stack, words: list(word)): result(string, stack) =>
  fold_left(eval_result, Ok(stack), words)
and eval_while_helper =
    (stack: stack, body: list(word), pred: list(word))
    : result(string, stack) =>
...
and eval_while =
  ...
  | Ok([b, ...st]) =>
    to_bool(b) ? eval_while_helper(st, body, pred) : Ok(st)
